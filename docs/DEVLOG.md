...
- (Milestone 10.1) The overlapping-multitouch swipe fix is verified against the real production code path via a direct headless simulation, but a live-browser confirmation of two real, overlapping touches was not achieved this session — this sandbox's CDP-driven touch automation showed enough run-to-run timing variance (countdown/boot duration varying several-fold between otherwise-identical runs) that neither attempt cleanly landed the gesture inside a confirmed-`ACTIVE` window. Consistent with the CDP-automation quirks already logged above, not a newly-discovered gameplay bug. Worth a real two-finger test on an actual phone.
